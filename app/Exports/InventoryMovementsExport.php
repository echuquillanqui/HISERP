...
class InventoryMovementsExport implements FromCollection, WithHeadings, ShouldAutoSize
{
    ...
    public function collection(): Collection
    {
        return InventoryMovement::query()
            ...
            ->when(!empty($this->filters['movement_type']), fn ($q) => $q->where('movement_type', $this->filters['movement_type']))
            ->when(!empty($this->filters['sources']), fn ($q) => $q->whereIn('source', $this->filters['sources']))
            ->when(!empty($this->filters['source']), fn ($q) => $q->where('source', $this->filters['source']))
