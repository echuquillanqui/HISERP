            ...
            <div class="col-lg-8">
                        ...
                        <template x-if="historyInfo">
                            <div :class="historyInfo.is_free ? 'alert alert-success' : 'alert alert-info'" class="mt-3 py-2 shadow-sm border-0 d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <template x-if="historyInfo.last_order_date">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white py-3 border-bottom text-primary fw-bold">
                        BÚSQUEDA DE EXÁMENES, PERFILES Y PRODUCTOS
                    </div>
                    <div class="card-body">
    ...
    return {
        ...
        initItemSelect() {
            ...
            this.itemSelect = new TomSelect('#item_select', {
                ...
                searchField: ['name', 'concentration', 'presentation', 'area', 'display_name'],
                maxOptions: 30,
                loadThrottle: 350,
                ...
                load: (q, cb) => {
                    ...
                    fetch(`/search-items?q=${encodeURIComponent(q)}`, { signal: this.itemSearchController.signal })
                        .then(r => r.json())
                        .then(j => cb(j.map(i => ({ ...i, uid: i.type+i.id, display_name: `${i.name}${productMeta(i)}${areaSuffix(i)} [${typeLabel(i.type)}]` }))))
                        .catch(() => cb());
                },
                render: {
