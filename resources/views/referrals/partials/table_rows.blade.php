@forelse($referrals as $referral)
    <tr>
        ...
        <td>
            <div class="fw-bold">{{ strtoupper(trim(($referral->patient->first_name ?? '') . ' ' . ($referral->patient->last_name ?? ''))) }}</div>
            <div class="text-secondary small">DNI: {{ $referral->patient->dni }}</div>
        </td>
        <td>
            @php $ins = strtoupper($referral->coverage_type ?? 'SIS'); @endphp
        ...
        <td class="text-end pe-4">
            <div class="btn-group">
                <a href="{{ route('referrals.edit', ['referral' => $referral->id, 'type' => $ins]) }}" class="btn btn-sm btn-outline-warning" title="Editar">
                    <i class="bi bi-pencil-square"></i>
                </a>
                
                <a href="{{ route('referrals.pdf', $referral->id) }}" class="btn btn-sm {{ $ins == 'SIS' ? 'btn-success' : 'btn-primary' }}" target="_blank" title="Imprimir {{ $ins }}">
